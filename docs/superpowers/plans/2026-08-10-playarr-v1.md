...
- [ ] **Step 4: Write a test that pins the DTO shapes**

This exists so the scaffold has a test to run and so a later rename of a DTO field fails loudly rather than silently diverging from the client.

`packages/shared/test/types.test.ts`:

```ts
import { describe, expect, it } from 'vitest';
import type { JobDto, SegmentRun, SettingsDto } from '../src/index.ts';

describe('shared DTOs', () => {
  it('describes a job that has been uploaded but not selected', () => {
    const job: JobDto = {
      id: 'j1',
  ...
  it('never carries a password in the settings DTO', () => {
  ...
  it('treats a segment run as half-open', () => {
    const run: SegmentRun = [0, 3];
    expect(run[1] - run[0]).toBe(3);
  });
});
```
...
Run: `npm install`
Then: `npm run check`
Expected: typecheck, lint, format:check all clean; 3 tests pass.
...
describe('Download failure handling', () => {
  ...
  it('treats a write failure as fatal and rejects parked readers', async () => {
    ...
    vi.spyOn(h.download as unknown as { writeSegment: () => Promise<void> }, 'writeSegment');

    // Replace the descriptor's write with one that always fails.
    const fd = (h.download as unknown as { fd: { write: unknown } }).fd;
    vi.spyOn(fd as { write: () => Promise<unknown> }, 'write').mockRejectedValue(enospc);
...
export class Download {
  ...
  get coveredBytes(): number {
    let total = 0;
    for (const [start, end] of this.#coverage.runs) {
      for (let index = start; index < end; index += 1) {
        total += this.#sizeOf(index);
      }
    }
    return total;
  }
    ...
    ...(selection === undefined
      ...
      : {
          selection: {
            ...
            dead: selection.dead,
            coveredBytes: coveredBytesOf(selection),
          },
        }),
  };
}

function coveredBytesOf(selection: NonNullable<JobRecord['state']['selection']>): number {
  const { segmentSize, lastSegmentSize, segmentCount } = selection.geometry;
  let total = 0;
  for (const [start, end] of selection.covered) {
    for (let index = start; index < end; index += 1) {
