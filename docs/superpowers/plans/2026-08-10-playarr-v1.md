...
describe('credentialsFor', () => {
  ...
  function resolve(secret: NntpSecret): Promise<string> {
    if (typeof secret === 'string') {
      throw new Error('expected a provider, got a literal');
    }
    return secret();
  }
...
The `@chad3814/secret-provider` surface, confirmed against the installed package —
note that a provider is a **callable**, not an object with a `.get()` method:

```ts
type Provider<T> = () => Promise<T>;
function chain<T>(...providers: Provider<T>[]): Provider<T>;
function fromEnv(name: string): Provider<string>;
function fromFile(path: string): Provider<string>;
function fromStatic<T>(value: T): Provider<T>;
function memoize<T>(provider: Provider<T>, isExpired?: (value: T) => boolean): Provider<T>;
```

Do not work around a resolution failure by reading the password eagerly — the whole
point is that the value is fetched when `AUTHINFO PASS` is built and never held
anywhere else. Do not memoize here either: `NntpPool` memoizes whatever it is given,
so a second layer would only widen the window the resolved secret sits in memory.
...
export const registerJobRoutes: FastifyPluginAsync<{ deps: AppDeps }> = async (
  ...
  app.get('/jobs', async () => store.list().map((record) => toJobDto(record, manager.activeId)));
...
export function App(): JSX.Element {
  ...
  const play = useCallback(async (jobId: string, fileIndex: number) => {
    setError(null);
    try {
