...
export class SegmentFetcher {
   ...
   * away every article in flight or drops the seek entirely.
   */
  ...
  async #pass(anchor: number): Promise<number | null> {
    ...
    try {
      for (;;) {
        ...
        const seek = this.#takeSeek(segment);
        if (seek !== null) {
          return seek;
        }
        ...
        const after = this.#takeSeek(segment);
        if (after !== null) {
          return after;
        }
      }
  ...
  /**
   * Take the pending demand, unless the pass is about to serve it anyway.
   * Re-anchoring throws away every article in flight, so a segment inside the
   * prefetch window is simply waited for.
   */
  #takeSeek(from: number): number | null {
    const target = this.#wanted;
    if (target === null) {
      return null;
    }
    this.#wanted = null;
    return target >= from && target < from + this.#prefetch ? null : target;
  }
