 ...
 * fetcher launched, so there is no caller to reject to and no request to
 * fail. `reportToStderr` is the fallback for callers with no logger handy
 * (mainly tests); `reportToLogger` is what the real entrypoint wires in.
 */
...
/**
 * The fallback destination, for callers with no logger available.
 *
 ...
 * is what an operator can actually find.
 */
